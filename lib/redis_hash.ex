defmodule RedisHash do
  ...
  * `:local_cache` - true/false to enable or disable local caching (default: true)
  ...
  """
  def push(%RedisHash{__local_changes__: changes} = container), do: do_push(Enum.empty?(changes), container)

  defp do_push(true, container), do: container
  defp do_push(false, %RedisHash{__redis_key__: key, __redis_adapter__: adapter, __binary_mode__: binary, __local_changes__: changes} = container) do
